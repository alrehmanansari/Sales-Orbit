...
const GROUPS = [
  {
    ...
    items: [
      { id: 'reports',       label: 'Reports',           icon: '📊', desc: 'Sales analytics & insights' },
      { id: 'customReports', label: 'Custom Reporting',  icon: '🔧', desc: 'Build & export tailored reports' },
    ],
  },
  {
    id: 'salesLib', label: 'Sales Library',
    items: [
...
export default function TopNav({ page, onNav, toggleTheme, colorScheme, isDark, setColorScheme, isMobile }) {
  ...
  useEffect(() => {
    ...
    if (!container) return
    const active = container.querySelector('[data-navactive="true"]')
    if (!active) return
    const pRect = container.getBoundingClientRect()
    const aRect = active.getBoundingClientRect()
    setPillStyle({ left: aRect.left - pRect.left, width: aRect.width, opacity: 1 })
  }, [page])
  ...
  useEffect(() => {
    function onDown(e) {
      ...
      if (!e.target.closest('[data-groupwrap]')) setOpenGroup(null)
    }
    ...
    <>
      ...
      <header style={{
        ...
        <div style={{
          ...
          {/* DIVIDER */}
          <div style={{ width: 1, height: 18, background: 'var(--border-strong-color)', opacity: 0.4, flexShrink: 0 }} />
          ...
          {!isMobile && (
            <div ref={navItemsRef} style={{ display: 'flex', alignItems: 'center', position: 'relative', flex: 1 }}>
              ...
              <span aria-hidden style={{
                ...
                background: isDark ? 'rgba(145,119,199,0.18)' : 'linear-gradient(90deg,rgba(71,150,227,0.10),rgba(145,119,199,0.10))',
                border: '1px solid var(--so-purple-soft)',
                pointerEvents: 'none',
                transition: 'left 0.36s cubic-bezier(0.2,0.8,0.2,1), width 0.36s cubic-bezier(0.2,0.8,0.2,1), opacity 0.2s',
              ...
              {FLAT_NAV.map(item => {
                ...
                return (
                  <button
                    key={item.id}
                    data-navactive={isActive ? 'true' : 'false'}
                    onClick={() => go(item.id)}
                    style={{
                  ...
                  >
                    ...
                    {item.badge && actionCount > 0 && (
                      <span style={{ minWidth: 16, height: 16, borderRadius: 8, background: 'var(--so-gradient)', color: '#fff', fontSize: 9, fontWeight: 800, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px' }}>
                        {actionCount}
                      </span>
                    )}
                  </button>
              ...
              <div style={{ width: 1, height: 16, background: 'var(--border-strong-color)', opacity: 0.4, margin: '0 4px', flexShrink: 0 }} />
              ...
              {GROUPS.map(group => {
                ...
                return (
                  <div key={group.id} data-groupwrap={group.id} style={{ position: 'relative' }}>
                    <button
                    ...
                    >
                      {group.label}
                      <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.5, transition: 'transform 0.2s', transform: isOpen ? 'rotate(180deg)' : 'none' }}>
                    ...
                    {isOpen && (
                      <div style={{
                        ...
                        boxShadow: isDark ? '0 24px 48px rgba(0,0,0,0.7), 0 0 0 1px rgba(145,119,199,0.15)' : '0 24px 48px rgba(31,38,135,0.15)',
                        animation: 'fadeUp 0.15s cubic-bezier(0.2,0.8,0.2,1) both',
                      }}>
                          ...
                          return (
                            <button key={item.id}
                              ...
                              style={{
                                ...
                                background: isAct ? 'linear-gradient(90deg,var(--so-blue-soft),var(--so-purple-soft))' : 'transparent',
                                cursor: 'pointer', fontFamily: 'var(--font)', textAlign: 'left',
                                transition: 'background 0.14s',
                              }}
                              onMouseEnter={e => { if (!isAct) e.currentTarget.style.background = isDark ? 'rgba(145,119,199,0.12)' : 'var(--bg-tertiary)' }}
                            ...
                            >
                              ...
                              <div>
                                <div style={{ fontSize: 13, fontWeight: 600, color: isAct ? 'var(--so-purple)' : 'var(--text-primary)' }}>{item.label}</div>
          ...
          {/* Divider */}
          {!isMobile && <div style={{ width: 1, height: 18, background: 'var(--border-strong-color)', opacity: 0.4, flexShrink: 0 }} />}

          {/* Search */}
          <div ref={searchRef} style={{ position: 'relative' }}>
            {searchOpen ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '5px 12px', borderRadius: 24, border: '1.5px solid var(--so-blue)', background: 'var(--bg-card)', boxShadow: '0 0 0 3px var(--so-blue-soft)', width: isMobile ? 180 : 240 }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--text-tertiary)" strokeWidth="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>
                <input ref={searchInput} value={query} onChange={e => handleSearch(e.target.value)} placeholder="Search…"
              ...
              <IconBtn onClick={() => { setSearchOpen(true); setTimeout(() => searchInput.current?.focus(), 60) }} title="Search (⌘K)">
            ...
            {searchOpen && results.length > 0 && (
              <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, width: 300, zIndex: 600, background: 'var(--bg-card)', border: '1px solid var(--so-blue)', borderRadius: 14, overflow: 'hidden', boxShadow: '0 8px 28px rgba(71,150,227,0.18)' }}>
                {results.map(r => (
                  <div key={r.id} onClick={() => go(r.nav)}
                    style={{ padding: '9px 14px', cursor: 'pointer', borderBottom: '0.5px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: 10, transition: 'background 130ms' }}
                    onMouseEnter={e => e.currentTarget.style.background = 'var(--so-blue-soft)'}
                    onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                    <span style={{ fontSize: 9, fontWeight: 700, color: r.type === 'lead' ? 'var(--so-blue)' : 'var(--so-purple)', background: r.type === 'lead' ? 'var(--so-blue-soft)' : 'var(--so-purple-soft)', padding: '2px 7px', borderRadius: 20, textTransform: 'uppercase', flexShrink: 0 }}>{r.type}</span>
          ...
          <div ref={themeRef} style={{ position: 'relative' }}>
            <IconBtn onClick={() => setThemeOpen(o => !o)} title="Theme" active={themeOpen}>
              <span style={{ width: 11, height: 11, borderRadius: '50%', background: activeScheme.color, display: 'block' }} />
            </IconBtn>
            {themeOpen && (
              <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, zIndex: 600, background: 'var(--bg-card)', border: '1px solid var(--border-strong-color)', borderRadius: 14, padding: 6, minWidth: 170, boxShadow: '0 8px 28px rgba(0,0,0,0.14)', animation: 'fadeUp 0.14s ease both' }}>
                ...
                {COLOR_SCHEMES.map(s => {
                  const isSel = s.id === colorScheme
                  return (
                    <button key={s.id} onClick={() => { setColorScheme(s.id); setThemeOpen(false) }}
                      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '7px 10px', borderRadius: 9, border: 'none', background: isSel ? 'var(--so-blue-soft)' : 'transparent', cursor: 'pointer', fontFamily: 'var(--font)', transition: 'background 120ms' }}
                      onMouseEnter={e => { if (!isSel) e.currentTarget.style.background = 'var(--bg-tertiary)' }}
                      onMouseLeave={e => { if (!isSel) e.currentTarget.style.background = 'transparent' }}>
                      <span style={{ width: 13, height: 13, borderRadius: '50%', background: s.color, flexShrink: 0, boxShadow: isSel ? `0 0 0 2px var(--bg-card), 0 0 0 3.5px ${s.color}` : 'none' }} />
                      <span style={{ fontSize: 13, fontWeight: isSel ? 600 : 400, color: isSel ? 'var(--so-blue)' : 'var(--text-primary)', flex: 1, textAlign: 'left' }}>{s.label}</span>
                      {isSel && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--so-blue)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>}
                    </button>
                  )
                })}
                <div style={{ height: '0.5px', background: 'var(--border-color)', margin: '5px 0' }} />
                <button onClick={() => { toggleTheme(); setThemeOpen(false) }}
                  style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '7px 10px', borderRadius: 9, border: 'none', background: 'transparent', cursor: 'pointer', fontFamily: 'var(--font)', fontSize: 13, color: 'var(--text-secondary)', transition: 'background 120ms' }}
          ...
          <div ref={userRef} style={{ position: 'relative' }}>
            <button onClick={() => setUserOpen(o => !o)} title={currentUser?.name}
              style={{ width: 30, height: 30, borderRadius: '50%', flexShrink: 0, border: '1.5px solid var(--so-blue)', background: 'var(--so-blue-soft)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font)', fontSize: 10, fontWeight: 700, color: 'var(--so-blue)', transition: 'all 0.15s' }}>
              {currentUser?.name?.split(' ').map(n => n[0]).join('').slice(0, 2) || 'U'}
            </button>
            {userOpen && (
              <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, zIndex: 600, background: 'var(--bg-card)', border: '1px solid var(--border-strong-color)', borderRadius: 14, padding: '8px 0', minWidth: 200, boxShadow: '0 8px 28px rgba(0,0,0,0.14)', animation: 'fadeUp 0.14s ease both' }}>
                ...
                <button onClick={() => { logout(); setUserOpen(false) }}
                  style={{ width: '100%', padding: '8px 14px', border: 'none', background: 'transparent', cursor: 'pointer', fontFamily: 'var(--font)', fontSize: 13, color: '#D93025', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8, transition: 'background 120ms' }}
          ...
          {isMobile && (
            <button onClick={() => setMobileOpen(o => !o)}
              style={{ width: 30, height: 30, borderRadius: 8, border: '1px solid var(--border-strong-color)', background: 'transparent', cursor: 'pointer', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>
              ☰
            </button>
          )}
        </div>
      ...
      {isMobile && mobileOpen && (
        <>
          <div onClick={() => setMobileOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(2px)', animation: 'fadeIn 0.18s ease' }} />
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, zIndex: 201, width: 280, background: 'var(--bg-secondary)', borderLeft: '0.5px solid var(--border-color)', display: 'flex', flexDirection: 'column', animation: 'slideInRight 0.24s cubic-bezier(0.34,1.36,0.64,1)', overflowY: 'auto' }}>
            <div style={{ padding: '14px 16px 10px', borderBottom: '0.5px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>Menu</span>
              <button onClick={() => setMobileOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-tertiary)', fontSize: 16 }}>✕</button>
            </div>
            <div style={{ padding: '8px 10px' }}>
              {[...FLAT_NAV, ...GROUPS.flatMap(g => g.items.map(i => ({ ...i, _group: g.label })))].map(item => {
                const isAct = page === item.id
                return (
                  <button key={item.id} onClick={() => go(item.id)}
                    style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: 'none', background: isAct ? 'linear-gradient(90deg,var(--so-blue-soft),var(--so-purple-soft))' : 'transparent', cursor: 'pointer', fontFamily: 'var(--font)', fontSize: 13, fontWeight: isAct ? 600 : 400, color: isAct ? 'var(--so-purple)' : 'var(--text-secondary)', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                    {item.icon && <span>{item.icon}</span>}
                    {item.label}
                    {item.badge && actionCount > 0 && <span style={{ marginLeft: 'auto', minWidth: 16, height: 16, borderRadius: 8, background: 'var(--so-gradient)', color: '#fff', fontSize: 9, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px' }}>{actionCount}</span>}
                    {item._group && <span style={{ marginLeft: 'auto', fontSize: 9, color: 'var(--text-hint)', letterSpacing: '0.5px', textTransform: 'uppercase' }}>{item._group}</span>}
                  </button>
                )
              })}
            </div>
